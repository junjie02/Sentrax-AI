...
def prepare_dataset(example: dict, student_tokenizer: PreTrainedTokenizer, teacher_tokenizer: PreTrainedTokenizer, config: dict) -> dict:
    """
    单样本预处理：将ShareGPT格式转换为模型输入格式（适配学生/教师tokenizer）
    Args:
        example: 单条数据集样本（需含"conversations"字段）
        student_tokenizer: 学生模型的tokenizer
        teacher_tokenizer: 教师模型的tokenizer
    ...
    teacher_enc = teacher_tokenizer(
        teacher_text, truncation=True, max_length=max_len, padding="max_length", return_tensors="pt"
    )
    ...
    result = {
        ...
        "teacher_input_ids": teacher_enc["input_ids"].squeeze(0).tolist(),
        "teacher_attention_mask": teacher_enc["attention_mask"].squeeze(0).tolist(),
    }

    return result


def preprocess_and_split_dataset(dataset, student_tokenizer, teacher_tokenizer, config: dict) -> DatasetDict:
    """
    批量预处理数据集 + 划分训练/测试集
    Args:
        dataset: 原始数据集（未预处理）
    ...
    features = Features({
        ...
        "teacher_attention_mask": Sequence(Value("int64")),
    })

    processed_dataset = dataset.map(
        lambda example: prepare_dataset(example, student_tokenizer, teacher_tokenizer, config),
